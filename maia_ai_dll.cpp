...
constexpr std::uintptr_t kExecuteMoveRva = 0x636D4;
...
SafetyHookInline g_executeMoveHook;
...
void __fastcall HookExecuteMove(Game* game, const GameMove* move) noexcept {
    ...
    if (turn != maiaSide) {
        g_runtime->Observe(board,
                           {(move->fromY << 4) + move->fromX,
                            ...
                            0},
                           elo);
        g_executeMoveHook.call<void>(game, move);
        return;
    }
    ...
    for (const auto& candidate : std::span{moves, static_cast<std::size_t>(count)})
        if (IsLegalMove(board, candidate))
            legalMoves.push_back(candidate);

    *const_cast<GameMove*>(move) = g_runtime->ChooseMove(board, legalMoves, elo);
    g_executeMoveHook.call<void>(game, move);
}
...
DWORD WINAPI MaiaBootstrapThread(LPVOID self) noexcept {
    try {
        ...
        auto executeMoveHook = safetyhook::create_inline(
            reinterpret_cast<void*>(g_moduleBase + kExecuteMoveRva),
            reinterpret_cast<void*>(&HookExecuteMove),
            SafetyHookInline::StartDisabled);
        if (!executeMoveHook)
            return 0;
        g_executeMoveHook = std::move(executeMoveHook);
        if (!g_executeMoveHook.enable())
            return 0;
